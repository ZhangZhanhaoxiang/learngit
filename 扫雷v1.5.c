 ...
 * 
 * 版本日志
 * v1.5 2020-11-26 使用多线程优化闪屏问题，增加一套文字方案解决emoji显示异常问题
 * v1.4 2019-11-8 解决屏闪问题，优化代码结构
 * v1.1 2017-6-27 解决了gcc++无法编译的bug 由于g++不支持变长数组做形参，故采用了全局变量做为地图
 * v1.2 2017-6-28 改进了所有变长数组，默认编译器可编译，原来4个方向移动光标改为8个方向，改进主菜单及整体配色方案
 * v1.3 2017-6-30 使用一套随机算法将第一下展开的格子周围的地雷随机分配到其他空白区域，保证首次展开的位置及周围一定是空格，以减少失败的运气成分
 */

// 调整显示符号可以把1改为0，即可体验emoji扫雷游戏，如果emoji显示不正常，请调整数量
#if 0
const char* map_str[] = {"＊","错","旗","??","  ",".."}; 
#else
const char* map_str[] = {"💣💣","❌","🚩🚩","❓","◽","⬜"};
#endif
...
//按键事件，控制移动
void* key_event(void* p); 
//全局变量，储存方格周围的地雷雷数字符串数组
const char* str[] = {"１", "２", "３", "４", "５", "６", "７", "８"};
...
int main(void)
{
    ...
    //启动按键事件线程监听按键
    pthread_create(&pid1, NULL, key_event, NULL);
	while (!(judge&&getch()))//主循环
	{
		draw_map();
		++game_time; //游戏时长
		usleep(1000000); //每次间隔0.1秒
	}
...
void draw_map()
{
	...
	printf("\033[43;34m 数字键控制光标移动, 5翻开, 0插旗或标记, #回主菜单\033[0m\n"); 
	for (int i = 0; i < w; i++)
		printf("--"); 
	printf("\n"); 
	...
	for (int i = 0; i <= h; i++)
	{
		for (int j = 0; j < w; j++)
		{
			...
			{
				case -1:
					printf("\033[41;30m%s\033[0m",map_str[0]); 
				break; 
				case -5:
					printf("\033[46;31m%s\033[0m",map_str[1]); 
					break; 
				case -4:
				if (x == j && y == i)
				...
				case -3:
					mine_num_r++; 
					if (x == j && y == i)
						printf("\033[47m%s\033[0m", map_str[2]); 
				else
					printf("\033[40m%s\033[0m", map_str[2]); 
				break; 
				case -2:
				if (x == j && y == i)
					printf("\033[47m%s\033[0m",map_str[3]); 
				else
					printf("\033[40;33m%s\033[0m",map_str[3]); 
				break; 
				case 0:
					mine_num_b++; 
				if (x == j && y == i)
					printf("\033[41m%s\033[0m",map_str[4]); 
				else
					printf("\033[46;5m%s\033[0m",map_str[5]); 
				break; 
				default:
				if (x == j && y == i)
		...
		if (i == h)
		{
			if (judge == 1)
				mine_num_b += mine_num; 
			printf("\n\033[40;33m时间:%2d\033[0m  \033[40;32m还剩%2d颗地雷\033[40;36m 进度:%0.2f%%  \033[37m难度:\033[0m%d*%d*%d", game_time, mine_num-mine_num_r, (double)(w*h-mine_num_b)*100/(w*h), w, h, mine_num); 
		}
		printf("\n"); 
	}
	...
	while (1)
	{
		...
		while (kbhit())	// 判断输入，有则循环
		{
			...
			case '0':			// 标记
				...
				else if (print_mine_map[y][x] == -3)
					print_mine_map[y][x] = -2;
				else if (print_mine_map[y][x] == -2)
